...
from app.breast_cancer_preprocessing import get_breast_cancer_train_test_data, load_breast_cancer_dataset

MODELS_DIR = os.path.join(BASE_DIR, "models", "breast_cancer")


def select_breast_cancer_quantum_features(X_train, y_train, X_test, feature_names, n_features=4, models_dir=MODELS_DIR):
    """
    Selects top n_features (4) from X_train using ANOVA F-value (SelectKBest).
    Fitted strictly on X_train to prevent data leakage.
    Saves selector object to backend/models/breast_cancer/selector.pkl.
    """
...
def build_breast_cancer_vqc(num_qubits=4, maxiter=50):
    """
...
def train_breast_cancer_vqc(vqc, X_train_q, y_train, models_dir=MODELS_DIR):
    """
...
def predict_breast_cancer_vqc(vqc, X_q):
    """
...
def evaluate_breast_cancer_vqc(vqc, X_test_q, y_test):
    """
    Evaluates trained Breast Cancer VQC model on test set.
    """
    y_test_np = y_test.to_numpy() if hasattr(y_test, "to_numpy") else np.array(y_test)
    y_pred, y_proba = predict_breast_cancer_vqc(vqc, X_test_q)
...
def run_breast_cancer_pipeline():
    """
    Executes complete Breast Cancer Hybrid QML Pipeline and prints all required verification outputs.
    """
    print("==========================================================================")
    print("   Breast Cancer Hybrid Quantum Machine Learning Model (VQC Pipeline)")
    print("==========================================================================")

    # 1. Load Preprocessed Data
    print("\n[1] Loading Breast Cancer Dataset & Performing Train/Test Split...")
    X_train, X_test, y_train, y_test, feature_names, feature_medians = get_breast_cancer_train_test_data()

    dataset_df = load_breast_cancer_dataset()
    print(f"    Dataset Shape:            {dataset_df.shape[0]} samples, {dataset_df.shape[1]} columns")
    print(f"    Feature Count:            {len(feature_names)} numerical features")
    print(f"    Train Shape:              {X_train.shape}")
    print(f"    Test Shape:               {X_test.shape}")

    # 2. Quantum Feature Selection
    print("\n[2] Performing Quantum Feature Selection (SelectKBest ANOVA F-value)...")
    X_train_q, X_test_q, selector, selected_features, selected_indices = select_breast_cancer_quantum_features(
    ...
    num_qubits = 4
    vqc, fmap, ansatz, optimizer, sampler = build_breast_cancer_vqc(num_qubits=num_qubits, maxiter=50)
    ...
    t0 = time.time()
    train_breast_cancer_vqc(vqc, X_train_q, y_train)
    t1 = time.time()
    ...
    metrics = evaluate_breast_cancer_vqc(vqc, X_test_q, y_test)

    print("\n" + "=" * 60)
    print("           BREAST CANCER VQC CONFUSION MATRIX")
    print("=" * 60)
    ...
    print("\n" + "=" * 60)
    print("     BREAST CANCER VARIATIONAL QUANTUM CLASSIFIER METRICS")
    print("=" * 60)
    ...
    sample_pred_raw, sample_proba_raw = predict_breast_cancer_vqc(vqc, sample_q_features)
    ...
    label_str = "Elevated Risk (Malignant)" if sample_pred == 1 else "Lower Risk (Benign)"
    true_label_str = "Elevated Risk (Malignant)" if sample_true_label == 1 else "Lower Risk (Benign)"
